...
Sub Main()
    ...
    'Check Files
    CheckFile App.Path + "\odyzlib.dll"
    CheckFile App.Path + "\odysseydll.dll"
    
    If Exists("tiles.rsc") Then Kill "tiles.rsc"
    If Exists("objects.rsc") Then Kill "objects.rsc"
    If Exists("sprites.rsc") Then Kill "sprites.rsc"
    If Exists("effects.rsc") Then Kill "effects.rsc"
    If Exists("hpbar.rsc") Then Kill "hpbar.rsc"
    If Exists("InterfaceLights.rsc") Then Kill "InterfaceLights.rsc"
    If Exists("stats.rsc") Then Kill "stats.rsc"
    If Exists("wait.rsc") Then Kill "wait.rsc"
    If Exists("menu.rsc") Then Kill "menu.rsc"
    If Exists("interface.rsc") Then Kill "interface.rsc"
    If Exists("atts.rsc") Then Kill "atts.rsc"
    For A = 1 To 28
        If Exists("mus" + CStr(A) + ".mid") Then Kill "mus" + CStr(A) + ".mid"
    Next A
    For A = 1 To 9
        If Exists("sound" + CStr(A) + ".wav") Then Kill "sound" + CStr(A) + ".wav"
    Next A
    
    CheckFile GetGfxPath + "tiles.rsc"
    CheckFile GetGfxPath + "objects.rsc"
    CheckFile GetGfxPath + "sprites.rsc"
    CheckFile GetGfxPath + "effects.rsc"
    CheckFile GetGfxPath + "hpbar.rsc"
    CheckFile GetGfxPath + "InterfaceLights.rsc"
    CheckFile GetGfxPath + "stats.rsc"
    CheckFile GetGfxPath + "wait.rsc"
    CheckFile GetGfxPath + "menu.rsc"
    CheckFile GetGfxPath + "interface.rsc"
    CheckFile GetGfxPath + "atts.rsc"
    ...
    While blnEnd = False
        ...
        If blnPlaying = True Then
            ...
            If Freeze = False Then
                ...
                MoveFloatText
                With Options
                    If .FrameRate = 2 Or Tick >= FrameTimer Then
                        DrawNextFrame
                        If .FrameRate < 2 Then FrameTimer = Tick + 24 - .FrameRate * 12
                    End If
                End With
            End If
...
Sub PrintInfoText(St As String, Optional Color As Long = 12)
    Dim A As Long
    InfoTextTimer = Tick
    InfoText(0) = InfoText(1)
    InfoTextColor(0) = InfoTextColor(1)
    InfoText(1) = InfoText(2)
    InfoTextColor(1) = InfoTextColor(2)
    InfoText(2) = InfoText(3)
    InfoTextColor(2) = InfoTextColor(3)
    InfoText(3) = St
    InfoTextColor(3) = Color
End Sub
...
Function IsVacant(X As Byte, Y As Byte) As Boolean
    If Character.Access > 0 And keyAlt = True Then
        IsVacant = True
        Exit Function
    End If
    Dim A As Long
        ...
        Else
            With EditMap.Tile(X, Y)
                ...
                ElseIf .FGTile > 0 Then
                    ...
                    Call FGTile2Buffer.BltFast(X * 32, Y * 32, DDSTiles, TileSource, DDBLTFAST_SRCCOLORKEY)
                End If
                If EditMode >= 6 Or frmMain.chkAtts Then
                    If .Att2 > 0 Then
...
Sub CheckKeys()
    ...
    If CX * 32 = CXO And CY * 32 = CYO Then
        If Character.Access = 0 Or keyAlt = False Then
            If CWalkStep > Runspeed And Character.Access = 0 Then
        ...
        End If
        If keyUp = True Then
            If CDir = 0 And Tick >= DirTimer Then
                If CY > 0 Then
        ...
        ElseIf keyDown = True Then
            If CDir = 1 And Tick >= DirTimer Then
                If CY < 11 Then
                    If IsVacant(CX, CY + 1) Then
        ...
        ElseIf keyLeft = True Then
            If CDir = 2 And Tick >= DirTimer Then
                If CX > 0 Then
        ...
        ElseIf keyRight = True Then
            If CDir = 3 And Tick >= DirTimer Then
                If CX < 11 Then
                    If IsVacant(CX + 1, CY) Then
...
Function NoDirectionalWalls(X As Byte, Y As Byte, Direction As Byte) As Boolean
    ...
    If Character.Access > 0 And keyAlt = True Then Exit Function
    Select Case Direction
    Case 0    'Up
        If Y >= 0 Then
            If Y > 0 Then
