...
Private Declare Function ZCompress Lib "odyzlib.dll" Alias "compress" (dest As Byte, destLen As Any, src As Any, ByVal srcLen As Long) As Long
Private Declare Function ZUncompress Lib "odyzlib.dll" Alias "uncompress" (dest As Byte, destLen As Any, src As Any, ByVal srcLen As Long) As Long
Private Declare Sub CopyMemory Lib "kernel32" Alias "RtlMoveMemory" (hpvDeSt As Any, hpvSource As Any, ByVal cbCopy As Long)
Private Declare Function Uncompress Lib "odyzlib.dll" Alias "uncompress" (ByRef DeStinationArray As Byte, ByRef destLen As Long, ByRef SourceArray As Byte, ByVal SourceLen As Long) As Long
Public Function ZCompressByteArray(ByRef ArrayToCompress() As Byte, _
    ...
    Else
        ...
        OrigSize = OrigSize & String$(11 - Len(OrigSize), Chr$(0))
        OrigSize = String$(5, Chr$(0)) & OrigSize
...
Public Function ZDecompressByteArray(ByRef ArrayToDecompress() As Byte, _
    ...
    Else
        ...
        Next
        If Left$(TestTag, 5) <> String$(5, Chr$(0)) Then
            Return_ErrorCode = -1
            Exit Function
        Else
            ' Get the original size from the tag value
            OrigSize = Right$(TestTag, Len(TestTag) - 5)
            OrigSize = Left$(OrigSize, InStr(OrigSize, Chr$(0)) - 1)
...
Public Function ZCompressFile(ByVal FileToCompress As String, _
    ...
    If FileToCompress = vbNullString Or OutputFile = vbNullString Then
        Exit Function
    ElseIf Dir(FileToCompress) = vbNullString Then
        Exit Function
...
Public Function ZDecompressFile(ByVal FileToDecompress As String, _
    ...
    ' Make sure the parameters passed are valid
    If FileToDecompress = vbNullString Or OutputFile = vbNullString Then
        Exit Function
    ElseIf Dir(FileToDecompress) = vbNullString Then
        Exit Function
    ElseIf Dir(OutputFile) <> "" And OverwriteFile = False Then
        Exit Function
    End If
