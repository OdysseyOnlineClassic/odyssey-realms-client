...
Begin VB.Form frmMain 
   ...
   Begin VB.PictureBox picMapEdit 
      ...
      End
   End
   Begin VB.Timer SkillTimer 
      Enabled         =   0   'False
      Interval        =   105
      Left            =   6720
      Top             =   7680
   End
   Begin VB.Frame storeframe 
      BorderStyle     =   0  'None
      Height          =   1935
      Left            =   3120
      TabIndex        =   162
      ...
      Visible         =   0   'False
      Width           =   3135
      Begin SHDocVwCtl.WebBrowser store 
         Height          =   1575
         Left            =   0
         TabIndex        =   192
         Top             =   360
...
Public SkillKeyDown As Long

Private Sub BrowserClose_Click()
frmMain.storeframe.Visible = False
End Sub

Private Sub chkAtts_Click()
    DrawMap
End Sub
                            ...
                            Case "SCAN"
                                If Character.Access >= 2 Then
                                    ...
                                    If Section(1) <> "" Then
                                        A = FindPlayer(Section(1))
                                        If A >= 1 Then
                                            SendSocket Chr$(18) + Chr$(18) + Chr$(A)
                                        Else
                                            PrintChat "Scan: No such player", 14
                                        End If
                                    Else
                                        PrintChat "Scan: Not enough parameters", 14
                                    End If
                                Else
                                    PrintChat "You do not have access to that command!", 14
                                End If
...
Private Sub lblMenu_MouseUp(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    Case 25    'Website Button
        ...
        frmMain.storeframe.Width = frmMain.Width
    
        frmMain.store.Width = frmMain.Width
        frmMain.BrowserClose.Width = frmMain.Width
        frmMain.store.Height = frmMain.Height - frmMain.BrowserClose.Height
        frmMain.storeframe.Visible = True
    Case 26    '
    ...
    Else
        ...
        If Button = 1 Then
            ...
            If FoundSomething = False Then
                Call PrintInfoText("You see nothing special.", 15)
...
Private Sub SkillTimer_Timer()
    If LastSkillUse + 500 < Tick Then
        If SkillKeyDown > 0 Then
            ListIndex = Character.Hotkey(SkillKeyDown - 111).Hotkey
            picList.Visible = True
            DrawMagicList
            Select Case Character.Hotkey(SkillKeyDown - 111).Type
            Case 1    'Skill
                DrawSkillsList
            End Select
            picList_DblClick
        ...
        Case "Magic"
            ...
            For A = 1 To MaxMagic
                If ExamineBit(Magic(A).Class, Character.Class - 1) And Character.Level >= Magic(A).Level Then
                    If Not (Magic(A).MagicLevel = 0 And Character.Access = 0 And ServerPort = 9999) Then
                        B = B + 1
                        If B > 0 Then
                            If B = ListIndex Then
