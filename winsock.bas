...
Function ConnectSock(ByVal host$, ByVal Port&, retIpPort$, ByVal HWndToMsg&, ByVal Async%) As Long
    ...
    Else
        ...
        End If
    End If
    'If setsockopt(s, SOL_SOCKET, SO_RCVBUF, 32728, 4) = SOCKET_ERROR Then
    '    If s > 0 Then
    '        dummy = closesocket(s)
    '    End If
    '    ConnectSock = INVALID_SOCKET
    '    Exit Function
    'End If
    'If setsockopt(s, SOL_SOCKET, SO_SNDBUF, 32768, 4) = SOCKET_ERROR Then
    '    If s > 0 Then
    '        dummy = closesocket(s)
    '    End If
    '    ConnectSock = INVALID_SOCKET
...
Function GetProtocolByName(ByVal protocol$) As Long
    ...
    If ppe = 0 Then
        ...
        If tmpShort <> 0 Or protocol = "0" Or protocol = vbNullString Then
...
Function GetSockAddress(ByVal s&) As String
    ...
    End If
    If Err Then GetSockAddress = vbNullString
End Function
'this function should work on 16 and 32 bit systems
...
Function SockaddressTOString(sa As sockaddr) As String
    ...
    If Err Then SockaddressTOString = vbNullString
End Function
